<p align="center">
   <img src="../../../.assets/banners/ansi-escape-sequences.gif" alt="banner" />
</p>

<h1 align="center">ANSI Escape Sequences</h1>
...
Arguments are delimited by a semi colon (`;`).
...
>   guaranteed to  work in all languages  and compilers. It is  recommended to
>   use the decimal, octal or hex representation as escape code.
...
>   representation (and input  method) and historically comes from  one of the
>   VT series of terminals.
...
>   `ESC[21m` sequence is  a non-specified sequence for  double underline mode
>   and only work in some terminals and is reset with `ESC[24m`.
...
> **Note:** the _Reset_  color is the reset code that  resets _all_ colors and
>   text effects, Use _Default_ color to reset colors only.
...
<p align="center">
   <img src="../../../.assets/figures/figure--ansi-escape-codes.png" alt="255 color ANSI escape code table" />
</p>
...
The final  24 colors (232-255)  are grayscale  starting from a  shade slightly
...
More modern terminals supports [True Color](https://en.wikipedia.org/wiki/Color_depth#True_color_.2824-bit.29) (24-bit RGB),
which allows to set foreground and background colors using RGB notation.
...
- `code` is  one or more  of the values listed  in the following  table. These
  values represent keyboard keys and key combinations. When using these values
  in a command, you  must type the semicolons shown in  this table in addition
  to the semicolons required by the  escape sequence. The codes in parentheses
  are not available on some keyboards. `ANSI.SYS` will not interpret the codes
  in parentheses for those keyboards unless you specify the `/X` switch in the
